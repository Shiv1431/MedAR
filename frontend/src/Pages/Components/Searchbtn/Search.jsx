...
function Search() {
  ...
  const openTeacherDec = async(id,fname,lname,sub)=>{
    ...
    const data = await fetch('http://localhost:8000/api/teacher/teacherdocuments',{
        method: 'POST',
        credentials: "include",
        headers: {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/course/student/${ID}/enrolled`, {
          method: 'GET',
          credentials: "include",
  ...
  const SearchTeacher = async (sub) => {
    ...
    try {
        const Data = await fetch(`http://localhost:8000/api/course/${subject}`, {
            method: "GET",
            credentials: "include", // Ensures cookies are sent with the request
            headers: {
                "Content-Type": "application/json",
                // If you're using JWT, send the token in headers
  ...
  const handleEnroll = async (courseName, id) => {
    let check = await fetch(
      `http://localhost:8000/api/course/${courseName}/${id}/verify/student/${ID}`,
      {
        method: "POST", 
        credentials: "include",
        headers: {
    ...
    const data = await fetch(`http://localhost:8000/api/payment/course/${id}/${courseName}`, {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
    ...
    const Key = await fetch("http://localhost:8000/api/payment/razorkey", {
      method: "GET",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        const verificationResponse = await fetch(
          `http://localhost:8000/api/payment/confirmation/course/${id}`,
          {
            method: "POST",
            credentials: "include",
            headers: {
        ...
        if (res.statusCode === 200) {
          try {
            let response = await fetch(
              `http://localhost:8000/api/course/${courseName}/${id}/add/student/${ID}`,
              {
                method: "POST",
                credentials: "include",
                headers: {
      ...
      prefill: {
        name: "Rishabh Pandey",
        email: "sigmarishabh29@gmail.com",
      },
      notes: {
      ...
      {openTM && (
          <div key='1' className='fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex items-center justify-center'>
              <div className='bg-[#008280] w-96 h-[21rem] rounded-md'>
                  ...
                  <div  className='flex flex-col justify-center p-5 text-1xl gap-4' style={{paddingBottom:"350px"}}>
