...
function StudentClasses() {
    ...
    useEffect(() => {
      const getData = async () => {
          try {
              const response = await fetch(`http://localhost:8000/api/course/classes/student/${ID}`, {
                  method: 'GET',
                  credentials: 'include',
                  headers: {
              ...
              console.log("API Response:", user); // 🔍 Debugging Step
  
              if (user?.data?.classes?.length > 0) {
                  console.log("Classes Found:", user.data.classes);
                  console.log("Live Classes:", user.data.classes[0]?.liveClasses);
